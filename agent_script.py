...
def delete_file(token, file_path):
    ...
    try:
        # שימוש ב-GET ובפרמטרים הנכונים: action=delete, what=file_path
...
def run_agent_on_audio(audio_data):
    ...
    try:
        ...
        print(f"Could not extract text from final response. Error: {e}")
        final_answer = "סיימתי את ביצוע המשימה."
